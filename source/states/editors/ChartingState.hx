...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	function addSongUI():Void
	{
		...
		{
			_song.player1 = bfVersions[Std.parseInt(character)];
			updateJsonData();
			updateHeads();
		});
		...
		var gfVersionDropDown = new FlxUIDropDownMenu(player1DropDown.x, player1DropDown.y + 40, FlxUIDropDownMenu.makeStrIdLabelArray(gfVersions, true), function(character:String)
		{
			_song.gfVersion = gfVersions[Std.parseInt(character)];
			updateJsonData();
			updateHeads();
		});
		...
		var player2DropDown = new FlxUIDropDownMenu(player1DropDown.x, gfVersionDropDown.y + 40, FlxUIDropDownMenu.makeStrIdLabelArray(dadVersions, true), function(character:String)
		{
			_song.player2 = dadVersions[Std.parseInt(character)];
			updateJsonData();
			updateHeads();
		});
