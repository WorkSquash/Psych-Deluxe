...
class ChartModifiersSubstate extends MusicBeatSubstate
{
	private var curOption:ChartModifier = null;
	...
	override function update(elapsed:Float)
	{
		...
		if(nextAccept <= 0)
		{
						...
						if(pressed) {
							...
							switch(curOption.type)
							{
								...
								case 'string':
									...
									{
										var oOption:ChartModifier = getOptionByName("Scroll Speed");
										if (oOption != null)
										{
			...
			if(controls.RESET)
			{
				for (i in 0...optionsArray.length)
				{
					var leOption:ChartModifier = optionsArray[i];
	...
	function updateTextFrom(option:ChartModifier) {
