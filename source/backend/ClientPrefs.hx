...
@:structInit class SaveVariables {
	...
	public var playVideos:Bool = false;
	...
	public var hitsound:String = 'Osu!';
...
class ClientPrefs {
	...
	public static function loadPrefs() {
		...
		for (key in Reflect.fields(data))
			if (key != 'modifiers' && Reflect.hasField(FlxG.save.data, key))
		...
		{
			var savedMap:Map<String, Dynamic> = FlxG.save.data.modifiers;
			for (name => value in savedMap)
				data.modifiers.set(name, value);
		}
	...
	inline public static function getModifiers(name:String, defaultValue:Dynamic = null, ?customDefaultValue:Bool = false):Dynamic
	{
		if(!customDefaultValue) defaultValue = defaultData.modifiers.get(name);
		return (data.modifiers.exists(name) ? data.modifiers.get(name) : defaultValue);
	}
